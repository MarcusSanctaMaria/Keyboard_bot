...
def not_blank(question):  # Defines the code as a not_blank function
                            # with question as parameter
    """
...
def check_string(question):
    # Defines the code as a check_string function with question as parameter
    """
    ...
    while True:  # Creates a while loop
        ...
        # Asks for user for string input with question as parameter
        x = response.isalpha()
...
def val_int(low, high, question):
    # Defines code as a val_int function with low, high, question as parameter
    """
    ...
    while True:  # Creates a while loop
        try:  # Triggers while the function is true
            ...
            # Creates an integer input for the question as a parameter
            if num >= low and num <= high:
                # Triggers if the input is greater to or equal to low (1) and
                # less than or equal to high (2)
...
def check_phone(question, PH_LOW, PH_HIGH):
    # Defines code as a check_phone function with
    # question, PH_LOW, PH_HIGH as parameter
    """
    Purpose: To make sure that the phone number that is input is an integer
             and is valid to be an NZ number
             (between 7 and 10 digits)
    Parameters: None
    Returns: None
    """
    while True:  # Creates a while loop
        try:  # Triggers while the function is true
            ...
            while test_num > 0:
                # Creates a loop while the test_num variable is 0
                test_num = test_num//10  # Divides the variable test_num by 10
                count = count + 1  # Adds 1 to the variable count
            if count >= PH_LOW and count <= PH_HIGH:
                # Triggers if the count is greater than PH_LOW and less than
                # or equal to PH_HIGH
...
def welcome():  # Defines code as a welcome function
    ...
    name = (names[num])
    # Name will be chosen from list with the
    # corresponding number taken from the randint above
    print("** Welcome to Karlos' Kool Keyboards **")
    # Prints a welcome message with asterisks
...
def order_type():  # Defines the code as an order_type function
    '''
    Purpose: To create a menu for the user to choose between
    ...
    # Sets the question variable to ask users
    # to choose between the LOW (1) and HIGH (2)
    ...
    # the user can only enter integers.
    # Uses LOW, HIGH and question as parameters
    if delivery == 1:  # Triggers if the input is 1
        print ("Click and Collect")
        # Prints Click and Collect to let the user know
        # they chose Click and Collect
        del_click = "Click and Collect"
        # Sets del_click as Click and Collect
        clickcollect_info()
...
def clickcollect_info():  # Defines the code as a clickcollect_info function
    """
    Purpose: To collect the users information when the user
    chooses Click and Collect
    Parameters: None
    Returns: None
    """
    question = ("Please enter your name ")
    # Sets question to ask the user for their name
...
def menu():  # Defines the code as a menu function
    ...
    """
    number_keyboard = 12
    # Sets number_keyboard variable to 12 as there are 12 items on the menu
    for count in range(number_keyboard):
        # For items within the range of 1 and 12
...
def order_keyboard():  # Defines code as an order_keyboard function
    ...
    for item in range(num_keyboards):
        ...
        # number of the keyboard in the menu)
        while num_keyboards > 0:
            # Creates while loop that continues while the input
            ...
            # between the LOW (1) and HIGH (12)
            keyboard_ordered = val_int(MENU_LOW, MENU_HIGH, question)
            # Sets keyboard_ordered to use the val_int fucntion to
            # make sure that the user can only enter integers.
            # Uses MENU_LOW, MENU_HIGH and question as parameters
            keyboard_ordered = keyboard_ordered - 1
            # Subtracts 1 from the keyboard_ordered variable
            #  to go down the list
            order_list.append(keyboard_names[keyboard_ordered])
            # Adds keyboard_names corresponding to the
            # keyboard_ordered number to the order_list
            order_cost.append(keyboard_prices[keyboard_ordered])
            # Adds keyboard_prices corresponding to the
            # keyboard_ordered number to the order_list
            print("{} ${:.2f}" .format(keyboard_names[keyboard_ordered],
                  keyboard_prices[keyboard_ordered]))
            # Prints out keyboard ordered with price
            num_keyboards = num_keyboards - 1
            # Subtracts 1 from the num_keyboards variable to go down the list


# Prints out order
def print_order(del_click):  # Defines function as a print_order function
    """
    Purpose: To print out the order. The order includes the
            customer details, order type, name and total order price.
    Parameters: del_click
    Returns: None
    """
    total_cost = sum(order_cost)
    # Sets the variable total_cost as the sum of numbers in the order_cost list
    ...
    if del_click == "Click and Collect":
        # Triggers if the order type is Click and Collect
        print("Thank you for your order, we'll send you"
              "a text message when it is ready!")
        # Prints message
    elif del_click == "Delivery":   # Triggers if the order type is delivery
        print ("Thank you for your order, it will be delievered to you soon!")
        # Prints message
...
def confirm_cancel():  # Defines the code as a confirm_cancel function
    """
    Purpose: To confirm if the user wants to continue their order or cancel it.
    Parameters: None
    Returns: None
    """
    question = (f"Enter a number between {LOW} and {HIGH} ")
    # Sets the question variable to ask users to choose
    # between the LOW (1) and HIGH (2)
    print("Please confrim your order.")  # Prints message
    print("For confirm please enter 1")
    # Prints message to let user know that confirm is 1
    print("For cancel enter 2")
    # Prints message to let user know that cancel is 2
    confirm = val_int(LOW, HIGH, question)
    # Sets num_keyboards to use the val_int fucntion
    # to make sure that the user can only enter integers.
    # Uses LOW, HIGH and question as parameters
    if confirm == 1:  # Triggers if the input is 1
        print("Order Confirmed.")  # Prints message
...
def new_exit():  # Defines the code as a new_exit function
    """
    Purpose: To give the user the option to
            start a new order or exit the program.
    Parameters: None
    Returns: None
    """
    question = (f"Enter a number between {LOW} and {HIGH} ")
    # Sets the question variable to ask users to choose
    # between the LOW (1) and HIGH (2)
    print("Do you want to start another order or exit?")  # Prints message
    print("To start another order, please enter 1")
    # Prints message to let user know that starting a new order is 1
    print("To exit the program, please enter 2")
    # Prints message to let user know exiting the program is 2
    order_exit = val_int(LOW, HIGH, question)
    # Sets new_exit to use the val_int fucntion
    # to make sure that the user can only enter integers.
    # Uses LOW, HIGH and question as parameters

    if order_exit == 1:  # Triggers if the order_exit variable is equal to 1
        print("New Order")  # Prints message
        order_list.clear()
        # Clears the order_list that was input throughout the program
        order_cost.clear()
        # Clears the order_cost that was input throughout the program
        customer_details.clear()
        main()  # Starts the main function to start the program again

    elif order_exit == 2:  # Triggers if the order_exit variable is equal to 2
        print("Exit")  # Prints message
        order_list.clear()
        # Clears the order_list that was input throughout the program
        order_cost.clear()
        # Clears the order_cost that was input throughout the program
        customer_details.clear()
        # Clears the customer_details that was input throughout the program
        sys.exit()  # Exits the program fully
...
main()  # Starts the main function to start the program
